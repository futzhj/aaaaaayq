...
static void on_wsarecv_complete(hio_t* io) {
    ...
    hoverlapped_t* hovlp = (hoverlapped_t*)io->hovlp;
    if (hovlp->bytes == 0) {
        // Connection closed by peer (TCP EOF).
        // NOTE: Do NOT call hio_close here directly — it frees hovlp which 
        // is still referenced by the caller (hio_handle_events).
        // Instead, set error and let hio_handle_events detect io->closed.
        io->error = WSAGetLastError();
        // Free the hovlp BEFORE hio_close to prevent double-free:
        // hio_close also tries to free io->hovlp.
        // For TCP recv, hovlp->buf.buf == io->readbuf.base (shared), so don't free it.
        io->hovlp = NULL;
        HV_FREE(hovlp);
        hio_close(io);
        return;
...
static void on_wsasend_complete(hio_t* io) {
    ...
    if (hovlp->bytes == 0) {
        io->error = WSAGetLastError();
        // Free hovlp before hio_close to prevent double-free
        HV_FREE(hovlp->buf.buf);
        io->hovlp = NULL;
        HV_FREE(hovlp);
        hio_close(io);
        return;  // Do NOT fall through to 'end' — hovlp is already freed
    }
    if (io->write_cb) {
        if (io->io_type == HIO_TYPE_UDP || io->io_type == HIO_TYPE_IP) {
            if (hovlp->addr) {
    ...
    // Normal completion: free send buffer and overlapped
    if (io->hovlp) {
        HV_FREE(hovlp->buf.buf);
        io->hovlp = NULL;
        HV_FREE(hovlp);
...
static void hio_handle_events(hio_t* io) {
    ...
    // CRITICAL: If the read callback triggered hio_close (e.g. bytes==0 EOF),
    // io->closed is now true. Do NOT proceed to process write events on a
    // closed IO object — the socket is already closed and hovlp freed.
    if (io->closed) {
        io->revents = 0;
        return;
    }
...
int hio_accept (hio_t* io) {
    io->accept = 1;
    // CRITICAL: Associate listen socket with IOCP BEFORE posting AcceptEx.
...
int hio_connect (hio_t* io) {
    ...
    // CRITICAL: Associate socket with IOCP BEFORE issuing ConnectEx.
    // On localhost, ConnectEx can complete instantly; if the socket isn't
    // yet associated with the IOCP, the completion notification is lost.
    io->connect = 1;
    ...
    if (ConnectEx(io->fd, io->peeraddr, sizeof(struct sockaddr_in6), NULL, 0, &dwbytes, &hovlp->ovlp) != TRUE) {
        int err = WSAGetLastError();
        if (err != ERROR_IO_PENDING) {
            fprintf(stderr, "ConnectEx error: %d\n", err);
            goto error;
        }
    }
    return 0;
...
int hio_read (hio_t* io) {
    // CRITICAL: Associate socket with IOCP BEFORE posting WSARecv.
    // Without this, WSARecv completion has no IOCP to deliver to.
    hio_add(io, hio_handle_events, HV_READ);
    post_recv(io, NULL);
    return 0;
}
...
int hio_close (hio_t* io) {
    ...
    io->closed = 1;
    hio_done(io);
    // IOCP fix: Do NOT free hovlp here. After CancelIo (called by hio_done),
    // the cancellation completion notification will still be posted to the IOCP.
    // GetQueuedCompletionStatus will return the OVERLAPPED* pointer — if we
    // free it now, that dereference becomes a use-after-free crash.
