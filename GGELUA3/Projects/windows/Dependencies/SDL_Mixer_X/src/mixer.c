...
Mix_Chunk * MIXCALLCC Mix_LoadWAV_RW(SDL_RWops *src, int freesrc)
{
    ...
    if (SDL_memcmp(magic, "WAVE", 4) == 0 || SDL_memcmp(magic, "RIFF", 4) == 0) {
        Sint64 wav_start = SDL_RWtell(src);
        wavfree = 1;
        loaded = SDL_LoadWAV_RW(src, SDL_FALSE, &wavespec, (Uint8 **)&chunk->abuf, &chunk->alen);
        if (!loaded) {
            /* SDL_LoadWAV_RW failed (e.g. unsupported WAVE format tag like GSM 6.10),
               fall back to music decoder chain which may handle more formats */
            SDL_RWseek(src, wav_start, RW_SEEK_SET);
            wavfree = 0;
            loaded = Mix_LoadMusic_RW(src, freesrc, &wavespec, (Uint8 **)&chunk->abuf, &chunk->alen);
            freesrc = SDL_FALSE; /* Mix_LoadMusic_RW took ownership if it succeeded */
        }
        if (freesrc) {
            SDL_RWclose(src);
        }
    ...
    if (wavespec.format != mixer.format ||
        ...
        if (wavecvt.len_cvt != wavecvt.len) {
            resized_buf = (Uint8 *)SDL_realloc(wavecvt.buf, wavecvt.len_cvt);
