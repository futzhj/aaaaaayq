...
read:
    ...
    if (nread == len && !io->closed) {
        // NOTE: When recv() returns exactly the buffer size, there may be
        // more data waiting in the kernel buffer. Continue reading to drain
        // it all in a single poll cycle. Without this, remaining TCP data
        // would be delayed until the next hloop_process_events() call,
        // causing visible NPC loading delays in the game client.
        // (SSL always did this; now TCP does too.)
        goto read;
    }
    return;
read_error:
...
#ifdef OS_WIN
    // Workaround: wepoll (IOCP-based epoll emulation) may miss the writable
    // transition if connect() completes before the fd is registered with wepoll.
    // This is common on localhost where TCP handshake is nearly instant.
    // Use a zero-timeout select() to detect if connect already completed.
    {
        fd_set writefds;
